package com.jinbostar.cpmp.heuristic_backups.gash;

import java.io.File;
import java.io.PrintStream;
import java.text.SimpleDateFormat;
import java.util.Calendar;
import java.util.Scanner;


public class GASH_LC
{

	public static GreedyAndSpeedy read_LC(File file) throws Exception
	{
		Scanner scn = new Scanner(file);
		scn.nextLine();
		int S = Integer.parseInt(scn.nextLine().split(" ")[2]);
		int H = Integer.parseInt(scn.nextLine().split(" ")[2]);
		int N = Integer.parseInt(scn.nextLine().split(" ")[2]);
		int maxPriority = 0;

		int[][] init = new int[S + 1][H + 1];

		for (int s = 1; s <= S; s++)
		{
			String[] str = scn.nextLine().split(" ");

			for (int i = 3; i < str.length; i++)
			{
				int t = i - 2;
				init[s][t] = Integer.parseInt(str[i]);
				assert init[s][t] >= 1;
				maxPriority = Math.max(maxPriority, init[s][t]);
			}
		}

		int P = maxPriority;

		scn.close();

		return new GreedyAndSpeedy(S, H, P, N, init);
	}

	public static void main(String[] args) throws Exception
	{
		String date = new SimpleDateFormat("yyyyMMdd-HHmmss").format(Calendar.getInstance().getTime());
		new File("result/" + date).mkdirs();


		File result = new File("result/" + date + "/result.csv");
		PrintStream ps = new PrintStream(result);

		ps.println("Group,Case,Move,Time");
		int tot_res_sum = 0;
		long tot_tim_sum = 0;
		int tot_cnt_sum = 0;
		for (int group = 1; group <= 5; group++)
		{
			String dir = "data/LC(5)/";
			String prefix = null;
			switch (group)
			{
				case 1:
					prefix = "LC1";
					break;
				case 2:
					prefix = "LC2a";
					break;
				case 3:
					prefix = "LC2b";
					break;
				case 4:
					prefix = "LC3a";
					break;
				case 5:
					prefix = "LC3b";
					break;
			}

			int res_sum = 0;
			long tim_sum = 0;
			int cnt_sum = 0;
			for (int ins = 1; ins <= (group == 1 ? 1 : 10); ins++)
			{

				File file = new File(dir + prefix + "/" + prefix.toLowerCase() + (group == 1 ? "" : "_" + ins) + ".bay");

				//	System.err.println(file.toString());

				GreedyAndSpeedy gash = read_LC(file);


				long st = System.currentTimeMillis();
				//		System.out.println(lpf.sideView());
				int res = gash.solve();
				//		System.out.println(lpf.sideView());
				long tim = System.currentTimeMillis() - st;


				//		ps.println(group + "," + ins + "," + res + "," + tim);
				//				System.out.println(group + "," + ins + "," + res + "," + tim);

				res_sum += res;
				tim_sum += tim;
				cnt_sum += 1;

			}

			System.out.println("Group " + group + ": " + String.format("%.2f", 1.0 * res_sum / cnt_sum) + " moves, " + String.format("%.2f", 1.0 * tim_sum / cnt_sum));
			tot_res_sum += res_sum;
			tot_tim_sum += tim_sum;
			tot_cnt_sum += cnt_sum;
		}
		System.out.println("Total: " + String.format("%.3f", 1.0 * tot_res_sum / tot_cnt_sum) + " moves, " + String.format("%.2f", 1.0 * tot_tim_sum / tot_cnt_sum));

		ps.close();
	}
}
