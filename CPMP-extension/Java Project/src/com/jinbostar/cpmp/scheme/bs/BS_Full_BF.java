...
public class BS_Full_BF
{

	public static Node read_BF(File file) throws Exception
	{
	...
	public static void main(String[] args) throws Exception
	{
		...
		ps.println("Group,Case,Move,Time,Iter,FoundAt");
		...
		for (int group = 1; group <= 32; group++)
		{
			...
			for (int ins = 1; ins <= 20; ins++)
			{
				...
				Node fbh = read_BF(file);

				assert fbh.check(S, H, N, P, M);

				fbh.topFilter = TrickyAvoid;
				fbh.stability = true;
				fbh.preferences = new TaskPreference[]{
						MoveActual, BlockingNiceInAimStack, DemandAffected, LowerAimTier, LargerPriority,
				};
			...
			//	System.out.println(S + "\t" + H + "\t" + N + "\t" + P + "\t" + M + "\t");
			System.out.println("BF" + group + " " + String.format("%.4f", 1.0 * res_sum / cnt_sum) + "\t" + String.format("%.2f", 1.0 * tim_sum / cnt_sum / 1000));
